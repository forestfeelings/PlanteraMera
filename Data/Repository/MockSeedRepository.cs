...
{
    public class MockSeedRepository: ISeedRepository
    {
        private List<Seed> seeds = new List<Seed>();
        private readonly ILogger<MockSeedRepository> _logger;

        public MockSeedRepository(ILogger<MockSeedRepository> logger)
        {
            _logger = logger;
            ...
            seeds.AddRange(allSeeds);
        }

        public IEnumerable<Seed> BeginnerSeeds
        {
            get
            {
                return seeds.Where(s => s.IsBeginnerSeed);
            }
        }
        ...
        public Seed GetSeedById(Guid id)
        {
            try
            {
                _logger.LogInformation("GetSeedById was called");

                return seeds.FirstOrDefault(s => s.SeedId == id);
            }
            catch (Exception ex)
            {
